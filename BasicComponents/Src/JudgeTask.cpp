...
void sendNumber(uint8_t number) {
	numberMsg[0] = '@';
	numberMsg[1] = number;
	if (HAL_UART_Transmit_IT(&RECV_UART, numberMsg, 2) != HAL_OK) { //这里需要改回中断。
		Error_Handler();
	}
}
...
void RecvRxCpltCallback() {
	...
	recvMsg[recvIndex] = tmpMsg;
	recvIndex = (recvIndex + 1) % 8;
	if (recvMsg[0] != 0x7A) recvIndex = 0;
	if (recvIndex == 7)
		if (recvMsg[7] == recvMsg[0] + recvMsg[1] + recvMsg[2] + recvMsg[3] + recvMsg[4] + recvMsg[5] + recvMsg[6]) {
